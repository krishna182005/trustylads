...
const GoogleAuthHandler: React.FC = () => {
  ...
  const handleGoogleAuth = useCallback(async () => {
    ...
    if (loginStatus === 'success') {
      try {
        ...
        if (response.ok) {
          ...
          if (payload?.success && userFromApi) {
            ...
            // Show success toast once per successful OAuth round-trip
            const alreadyToasted = sessionStorage.getItem('googleSuccessToast');
            if (!alreadyToasted) {
              toast.success('Successfully signed in with Google!');
              sessionStorage.setItem('googleSuccessToast', '1');
            }
          }
      ...
      // Avoid duplicate error toasts if user refreshes on callback page
      const alreadyFailed = sessionStorage.getItem('googleFailedToast');
      if (!alreadyFailed) {
        toast.error('Google authentication failed. Please try again.');
        sessionStorage.setItem('googleFailedToast', '1');
      }
      navigate('/login', { replace: true });
    } else {
