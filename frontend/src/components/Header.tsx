    ...
    <header className="bg-black shadow-lg sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <Link to="/" className="flex items-center space-x-2" onClick={() => { closeMobileMenu(); window.scrollTo({ top: 0, behavior: 'smooth' }); }} aria-label="Go to homepage">
            <img
              src="/logo.svg"
              alt="TrustyLads logo"
              width={40}
            ...
            <span className="text-xs sm:text-sm text-gray-300 hidden lg:block">- Your Lifestyle Store</span>
          </Link>
          ...
          <nav className="hidden md:flex items-center space-x-2 lg:space-x-4 xl:space-x-8">
            {navigationItems.map((item) => (
              <Link 
                key={item.path}
                to={item.path} 
                className={`font-medium transition-colors text-xs lg:text-sm xl:text-base ${
          ...
          <div className="hidden md:flex items-center space-x-2 lg:space-x-4">
            ...
            <Link
              ...
              aria-label={`View cart${itemsCount > 0 ? `, ${itemsCount} items` : ''}`}
              onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}
            >
            ...
            {isAuthenticated ? (
              <div className="flex items-center space-x-2">
                <span className="text-xs lg:text-sm text-white hidden lg:block">
                  Hi, {user?.name || 'User'}
                </span>
                <button
              ...
              <div className="border-t border-gray-700 pt-4">
                {isAuthenticated ? (
                  <div className="space-y-3">
                    ...
                    {/* My Orders Link - Mobile (only show if not already in navigationItems) */}
                    {!navigationItems.some(item => item.path === '/orders') && (
                      <Link
                        to="/orders"
                        onClick={() => { closeMobileMenu(); window.scrollTo({ top: 0, behavior: 'smooth' }); }}
                        className="flex items-center space-x-3 px-4 py-3 text-white hover:bg-gray-800 hover:text-yellow-400 rounded-lg transition-colors"
