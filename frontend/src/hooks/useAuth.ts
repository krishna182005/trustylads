...
export const useAuth = () => {
  ...
  const logoutMutation = useMutation({
    mutationFn: async () => {
      // Use the correct endpoint for user logout
  ...
  const setupTokenRefresh = () => {
    if (!token) return;
    // Skip refresh for non-JWT tokens (e.g., Google OAuth marker) or malformed tokens
    if (token === 'google-oauth' || !token.includes('.')) return;
    ...
    try {
      ...
      // Silently ignore if token can't be decoded (e.g., not a JWT)
    }
  };
