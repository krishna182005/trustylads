...
const CheckoutPage: React.FC = () => {
  ...
  const { checkVerification } = useEmailVerification();
  ...
  // Temporarily disable shipping cost for guest users (show as struck-through)
  const shipping = !isAuthenticated ? 0 : (isEligibleForFreeDelivery ? 0 : 99); // After first 5 orders, always ₹99 shipping
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (checkoutData.paymentMethod === 'razorpay') {
        // Check email verification for guest users
        if (!isAuthenticated && !isEmailVerified) {
          console.log('🔄 Checking email verification...');
          const verificationResult = await checkVerification(checkoutData.customer.email);
          ...
          console.log('✅ Email verification passed, proceeding to Razorpay...');
        }
        ...
        let response;
        try {
          console.log('🔄 About to call API client...');
          response = await apiClient.post<any>('/api/payments/razorpay/create', {
            amount: total,
            orderId: `TL${Date.now()}`,
            customer: checkoutData.customer
          });
        ...
        console.log('🔍 Response being passed to Razorpay:', response);
        ...
        if (!response.order) {
          console.error('❌ Missing order data in response:', response);
          toast.error('Payment order data is missing. Please try again.');
          setLoading(false);
          return;
        }
    ...
    return null;
     ...
     <div className="min-h-screen bg-gray-50 py-8 pb-16 sm:pb-20">
       <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="lg:col-span-2">
            ...
            >
              <div className="flex items-center justify-between overflow-x-auto no-scrollbar -mx-2 px-2 sm:overflow-visible sm:mx-0 sm:px-0">
                {steps.map((step, index) => (
                  <div key={step.number} className="flex items-center flex-shrink-0 mr-2 sm:mr-4 last:mr-0 sm:last:mr-0">
                    <div className={`flex items-center justify-center w-8 h-8 sm:w-8 sm:h-8 rounded-full border-2 ${
                      step.completed ? 'bg-green-500 border-green-500 text-white' :
                      currentStep === step.number ? 'bg-yellow-400 border-yellow-400 text-black' :
                      'bg-white border-gray-300 text-gray-500'
                    }`}>
                      {step.completed ? <Check className="h-4 w-4 sm:h-4 sm:w-4" /> : step.number}
                    </div>
            ...
            >
              ...
              {currentStep === 4 && (
                <div>
                  ...
                  <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 rounded-lg">
                    <h3 className="font-medium mb-2 text-sm sm:text-base">Contact Information</h3>
                    <div className="space-y-1">
                      <p className="text-xs sm:text-sm text-gray-600 break-words">{checkoutData.customer.email}</p>
                      <p className="text-xs sm:text-sm text-gray-600">{checkoutData.customer.phone}</p>
                    </div>
                  </div>
                  ...
                  <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-xs sm:text-sm text-gray-600 space-y-1">
                      <p className="break-words">{checkoutData.shipping.firstName} {checkoutData.shipping.lastName}</p>
                      <p className="break-words">{checkoutData.shipping.address}</p>
                      {checkoutData.shipping.apartment && <p className="break-words">{checkoutData.shipping.apartment}</p>}
                      <p className="break-words">{checkoutData.shipping.city}, {checkoutData.shipping.state} - {checkoutData.shipping.pinCode}</p>
                      <p className="break-words">{checkoutData.shipping.country}</p>
                    </div>
                  </div>
                  ...
                  <div className="mb-4 sm:mb-6">
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      {items.map((item) => (
                        <div key={`${item.productId}-${item.size}`} className="flex items-start space-x-2 sm:space-x-3 p-2 sm:p-3 bg-gray-50 rounded-lg">
                          <img
                            src={item.image || '/api/placeholder/50/60'}
                            alt={item.name}
                            className="w-10 h-12 sm:w-12 sm:h-15 object-cover rounded flex-shrink-0"
                          />
                          ...
                          <p className="font-medium text-xs sm:text-sm flex-shrink-0">₹{(item.price * item.quantity).toLocaleString()}</p>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-0 mt-6 sm:mt-8">
                <button
                  onClick={handleBack}
                  className="px-4 sm:px-6 py-3 sm:py-2 text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors text-sm sm:text-base"
                >
                  Back
                </button>
                ...
                {currentStep < 4 ? (
                  <button
                    onClick={handleNext}
                    className="px-4 sm:px-6 py-3 sm:py-2 bg-yellow-400 text-black font-medium rounded-lg hover:bg-yellow-500 transition-colors text-sm sm:text-base"
                  >
                    Continue
                  </button>
                ) : (
                  <button
                    ...
                    disabled={loading}
                    className="px-6 sm:px-8 py-3 sm:py-2 bg-black text-white font-medium rounded-lg hover:bg-gray-800 transition-colors disabled:opacity-50 text-sm sm:text-base"
                  >
                    {loading ? (
            ...
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 sticky top-4 sm:top-8">
              ...
              <div className="space-y-2 sm:space-y-3 mb-4">
                {items.slice(0, 3).map((item) => (
                  <div key={`${item.productId}-${item.size}`} className="flex items-center space-x-2 sm:space-x-3">
                    <img
                ...
                {items.length > 3 && (
                  <p className="text-xs sm:text-sm text-gray-500">+{items.length - 3} more items</p>
                )}
              </div>
        ...
        <motion.div 
          className="fixed bottom-0 left-0 right-0 bg-gradient-to-r from-green-500 via-blue-500 to-purple-500 text-white py-2 sm:py-2 overflow-hidden z-50"
          initial={{ y: 100 }}
          animate={{ y: 0 }}
          transition={{ duration: 0.5, delay: 1 }}
        >
          <div className="flex items-center justify-center space-x-1 sm:space-x-2 md:space-x-4 lg:space-x-8 text-xs sm:text-sm font-medium animate-pulse px-1 sm:px-2">
            <div className="flex items-center">
