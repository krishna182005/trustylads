...
export const getSafeImageUrl = (imageUrl: string, fallbackUrl?: string): string => {
  if (!imageUrl) {
    return fallbackUrl || 'https://ui-avatars.com/api/?name=Product&background=f3f4f6&color=9ca3af&size=300x400';
  }
  ...
  if (imageUrl.startsWith('/')) {
    return `https://trustylads.tech${imageUrl}`;
  }
...
export const handleImageError = (event: React.SyntheticEvent<HTMLImageElement, Event>, fallbackUrl?: string) => {
  ...
  if (img.src !== fallbackUrl) {
    img.src = fallbackUrl || 'https://ui-avatars.com/api/?name=Product&background=f3f4f6&color=9ca3af&size=300x400';
  }
};
